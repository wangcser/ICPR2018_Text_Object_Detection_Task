...
# define the NN Init and loss func.
...
class YOLONet(object):
    ...
    def __init__(self, is_training=True):
        ...
        self.output_size = (self.cell_size * self.cell_size) *\
            (self.num_class + self.boxes_per_cell * 5)  # S*S*(B*5)
        #
        self.scale = 1.0 * self.image_size / self.cell_size # 每个cell的像素大小  pixel

        # 整体类别信息的张量维度 0, 每个bnd 的大小
        self.boundary1 = self.cell_size * self.cell_size * self.num_class
        # 整体boundingBox的总合 S*S*20+S*S*2
        ...
        if is_training:
            ...
            # 输出张量 batch_size*S*S*(5) 只输出一个boundingBox
    ...
    def build_network(self,
        ...
        with tf.variable_scope(scope):
            with slim.arg_scope(
                ...
                activation_fn=leaky_relu(alpha),    # leaky_ReLU
    ...
    def calc_iou(self, boxes1, boxes2, scope='iou'):
        ...
        with tf.variable_scope(scope):
            ...
            # thus the box must be the square
            # 修改这里可能能够实现非矩形区域的识别
    ...
    def loss_layer(self, predicts, labels, scope='loss_layer'):
        # predicts 1470 = (7*7*10) in one hot vector
        # labels = 7*7*5
        ...
        with tf.variable_scope(scope):
            ...
            # 将预测结果 的 1 ~ 2 转换为每个 bnd 的对 obj 的响应值
            ...
            # 每个 cell 中 boxes 的 obj 的响应值
            response = tf.reshape(
                labels[..., 0],
                [self.batch_size, self.cell_size, self.cell_size, 1])

            # 每个 cell 中 boxes 的坐标 为 5 维张量
            # boxes = 16*7*7*1(box)*4(coordinator)
            boxes = tf.reshape(
                labels[..., 1:5],
                [self.batch_size, self.cell_size, self.cell_size, 1, 4])

            # tile 用于张量复制，将 box 复制 bnd_num 份，给每个 bnd 一个参考
            # 并对 box 大小（长和宽）做归一化
